...
X = np.loadtxt('../test_set/exp1/1_X.txt')
Y = np.loadtxt('../test_set/exp1/1_Y.txt')

X = X.reshape(X.shape[0], 1, height_red, width_red)
Y = Y.reshape(Y.shape[0], 1, height_red, width_red)

Y[Y > 0] = 1
prediction = model.predict(X)
actual = Y.reshape((Y.shape[0], height_red*width_red))
...
for threshold in x_val:
    ...
    for i in range(prediction.shape[0]):
        result = thresholded_prediction[i]
